...
class THINGSTriplet(torch.utils.data.Dataset):
    def __init__(
        self, root, aligned=True, transform=None, target_transform=None, download=True, cc0=False
    ):
    ...
    def __getitem__(self, index: int) -> Tuple[Tensor, Tensor, Tensor, int]:
        ...
        images = []
        for idx in triplet:
            img = os.path.join(self.root, "cc0_images" if self.cc0 else "images", self.names[idx])
            img = Image.open(img)
...
class THINGSBehavior(torch.utils.data.Dataset):
    def __init__(
        self, root, aligned=True, transform=None, target_transform=None, download=True, cc0=False, return_names=False
    ):
