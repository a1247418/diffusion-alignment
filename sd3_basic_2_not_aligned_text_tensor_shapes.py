...
# ------------------------
# Shape logging utilities
# ------------------------

def _format_dim_semantics(t: torch.Tensor) -> str:
    """
    Provide human-readable semantics for common tensor layouts.
    """
    if not isinstance(t, torch.Tensor):
        return "not a tensor"
    if t.ndim == 4:
        return "dim0=batch, dim1=channels, dim2=height, dim3=width"
    if t.ndim == 3:
        token_axis = 1 if t.shape[1] >= t.shape[2] else 2
        if token_axis == 1:
            return "dim0=batch, dim1=tokens, dim2=embedding"
        return "dim0=batch, dim1=embedding, dim2=tokens"
    if t.ndim == 2:
        return "dim0=batch, dim1=features"
    return f"ndim={t.ndim}, semantics=unknown"


def _log_output_tensor_shapes(out) -> None:
    """
    Print shapes (and semantic labels) of tensors found in a typical SD3 block output.
    Scans common containers: Tensor, dict (including shallow lists/tuples), list/tuple.
    """
    lines = []

    def record(label: str, t: torch.Tensor):
        try:
            shape = tuple(t.shape)
        except Exception:
            shape = "<?>"
        lines.append(f" - {label}: shape={shape} ({_format_dim_semantics(t)})")
    ...
    if lines:
        tqdm.write("Hidden state tensor shapes:")
        for line in lines:
            tqdm.write(line)
...
class SD3HighestBlockEncoder(torch.nn.Module):
    def __init__(
        ...
        self._did_log_shapes = False  # log shapes only once per encoder lifetime
        ...
        *,
        ...
        def hook_fn(_m, _inp, out):
            if not getattr(self, "_did_log_shapes", False):
                try:
                    _log_output_tensor_shapes(out)
                except Exception as e:
                    tqdm.write(f"Failed to log tensor shapes: {e}")
                self._did_log_shapes = True
            sel = _select_tensor_from_output(out)
            if sel is not None:
                try:
                    tqdm.write(
                        f"Selected tensor: shape={tuple(sel.shape)} ({_format_dim_semantics(sel)})"
                    )
                except Exception:
                    pass
