...
# Pooling FIX: for SD3 MMDiT blocks shaped (B, L, D), we pool over the TOKEN axis (dim=1).
...
# Local modules expected in your env
...
def token_mean_pool(x: torch.Tensor) -> torch.Tensor:
    """
    SD3 MMDiT block outputs are (B, L, D): tokens x embedding.
    We must pool over the TOKEN axis to produce (B, D).
    """
    if x.ndim == 3:
        # (B, L, D) -> mean over L
        return x.mean(dim=1)
    # Fallbacks for unexpected shapes (keep behavior sane):
    # - If it's already (B, D), return as-is.
    # - If it has more dims, reduce non-batch dims sensibly.
    if x.ndim == 2:
        return x
    # Generic conservative fallback: reduce all dims except batch by mean.
...
class SD3TransformerProber:
    ...
    def forward_one_block(
        ...
        # Ensure timestep is [B] and float
        ...
        # >>> POOL OVER TOKENS (dim=1) <<<
        feats = token_mean_pool(activations[0].float())  # (B, D)
        return feats
