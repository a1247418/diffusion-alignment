        ...
        for idx, sample in enumerate(tqdm(loader, total=len(dataset), desc="Extracting features", unit="img")):
            ...
            if save_k and save_k > 0 and idx < save_k:
                ...
                try:
                    ...
                    # Prepare image tensor in [-1, 1] for VAE
                    img_tensor = img.to(device=encoder.device, dtype=encoder.pipe.vae.dtype)
                    if img_tensor.ndim == 3:
                        img_tensor = img_tensor.unsqueeze(0)
                    img_tensor = img_tensor.clamp(0, 1) * 2.0 - 1.0

                    # Encode to latents
                    scaling_factor = getattr(encoder.pipe.vae.config, "scaling_factor", 0.18215)
                    posterior = encoder.pipe.vae.encode(img_tensor).latent_dist
                    latents = posterior.sample(generator=encoder.generator) * scaling_factor

                    # Compute timestep corresponding to strength
                    encoder.pipe.scheduler.set_timesteps(num_inference_steps, device=encoder.device)
                    timesteps = encoder.pipe.scheduler.timesteps
                    init_timestep = min(int(num_inference_steps * strength_used), num_inference_steps)
                    t_start = max(num_inference_steps - init_timestep, 0)
                    timestep = timesteps[t_start]

                    # Add noise and decode back to pixel space
                    # Use torch.randn without generator for broader PyTorch compatibility
                    noise = torch.randn(latents.shape, device=latents.device, dtype=latents.dtype)
                    noised_latents = encoder.pipe.scheduler.add_noise(latents, noise, timestep)
                    decoded = encoder.pipe.vae.decode(noised_latents / scaling_factor)
                    sample = decoded.sample if hasattr(decoded, "sample") else decoded
                    sample = (sample / 2.0 + 0.5).clamp(0, 1)
                    sample_img = sample[0].detach().cpu()
